...
api_key<-jsonlite::fromJSON(here::here("reports","quandl_api.json"))[[1]]
...
if (!file.exists(here::here("data","si_f.RDS"))) {
  ...
  SI_F <- price_bind(SI_F, SI_F_New)
  GC_F <- price_bind(GC_F, GC_F_New)
  
  GVS_F<-zoo::xts(order.by = index(SI_F))
  
  GVS_F$Close<-Cl(GC_F)/Cl(SI_F)
  GVS_F$Open<-Op(GC_F)/Op(SI_F)
  GVS_F$High<-Hi(GC_F)/Hi(SI_F)
  GVS_F$Low<-Lo(GC_F)/Lo(SI_F) 
  GVS_F<-GVS_F[complete.cases(Cl(GVS_F)),]
  ...
  GVS_F<-xts(order.by = index(SI_F))
  GVS_F <- readRDS(here::here("data","gvs_f.RDS"))
  
  # if(max(index(SI_F))<Sys.Date() & as.POSIXlt(Sys.Date())$wday %in% 1:5){
    GC_F_New <- quantmod::getSymbols("GC=F", auto.assign = F)
    SI_F_New <- quantmod::getSymbols("SI=F", auto.assign = F)
      SI_F <- price_bind(SI_F, SI_F_New)
  GC_F <- price_bind(GC_F, GC_F_New)
...
silver_mean<-as.numeric(last(Cl(GC_F)))/round(avg_gvs,2)
